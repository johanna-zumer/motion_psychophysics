...
%
% INPUT:  s.el.dummy    -    contains a logical, whether or not to use
%                                 dummy mode for initialization
%  s.el.online
%  s.el.window (window index)
%  s.grey
%  s.white
%  s.subid
%  s.res
...
%   
% obtained from Steffen Buergers (4 April, 2016) 
% modified by Johanna Zumer
...
% Settings
s.el = EyelinkInitDefaults_jz(s.el.window, s.el);

keyboard
s.el.backgroundcolour        = s.grey;
s.el.msgfontcolour           = s.white;
s.el.imgtitlecolour          = s.white;
s.el.calibrationtargetcolour = s.white;
...
disp(['setup.el.devicenumber is ' num2str(s.el.devicenumber)])

EyelinkUpdateDefaults(s.el);

keyboard
% Initialization of the connection with the eyetracker.
s.el.online = EyelinkInit(s.el.dummy, 1);
if ~s.el.online
...
% edf link
s.edfFile = sprintf('subj%s.edf',s.subid);
res = Eyelink('Openfile', s.edfFile);
Eyelink('Command', 'add_file_preamble_text = "DFI experiment recording of participant %s', s.subid);
if res~=0
...
if Eyelink('IsConnected')~=1 && ~s.el.dummy
    cleanup;
    return;
end
...
Eyelink('Command','screen_pixel_coords = %ld %ld %ld %ld', 0, 0, s.res(1)-1, s.res(2)-1);
Eyelink('message','DISPLAY_COORDS %ld %ld %ld %ld',        0, 0, s.res(1)-1, s.res(2)-1);
